...
- (void)parser:(__unused NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName attributes:(NSDictionary *)attributeDict{
	...
	switch (_nodeNameMode){
        ...
        case XMLDictionaryNodeNameModeAlways:{
            node[XMLDictionaryNodeNameKey] = elementName;break;
        }
        case XMLDictionaryNodeNameModeNever:break;
	}
	if ([attributeDict count]){
        switch (_attributesMode){
            ...
            case XMLDictionaryAttributesModeDictionary:{
                node[XMLDictionaryAttributesKey] = attributeDict;break;
            }
            case XMLDictionaryAttributesModeUnprefixed:{
                [node addEntriesFromDictionary:attributeDict];break;
            }
            case XMLDictionaryAttributesModeDiscard:break;
        }
	}
	if (!_root){
        _root = node;
...
+ (NSDictionary *)dictionaryWithXMLParser:(NSXMLParser *)parser{return [[[XMLDictionaryParser sharedInstance] copy] dictionaryWithParser:parser];}
+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data{return [[[XMLDictionaryParser sharedInstance] copy] dictionaryWithData:data];}
+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string{return [[[XMLDictionaryParser sharedInstance] copy] dictionaryWithString:string];}
+ (NSDictionary *)dictionaryWithXMLFile:(NSString *)path{return [[[XMLDictionaryParser sharedInstance] copy] dictionaryWithFile:path];}
...
- (id)innerText{
	...
	if ([text isKindOfClass:[NSArray class]]) return [text componentsJoinedByString:@"\n"];
	else return text;
}
- (NSString *)innerXML{
