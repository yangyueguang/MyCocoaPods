...
-(instancetype)initWithQQWalletappid:(NSString *)appid
    ...
    if(self = [super init]){
        [self.dic setValue:appid forKey:@"appid"];
        [self.dic setValue:mch_id forKey:@"mch_id"];
        [self.dic setValue:nonce_str forKey:@"nonce_str"];
        [self.dic setValue:body forKey:@"body"];
        [self.dic setValue:out_trade_no forKey:@"out_trade_no"];
        [self.dic setValue:fee_type forKey:@"fee_type"];
        [self.dic setValue:total_fee forKey:@"total_fee"];
        [self.dic setValue:spbill_create_ip forKey:@"spbill_create_ip"];
        [self.dic setValue:trade_type forKey:@"trade_type"];
        [self.dic setValue:notify_url forKey:@"notify_url"];
        _wechatPartnerKey = appkey;
        [self createMd5Sign:self.dic];
    }
    return self;
}
                       ...
                       timestamp:(UInt32)timestamp_key
{
    ...
    NSString *WechartSafeID = @"xuechao123456789123456789xuechao";// 安全校验码（MD5）密钥，商户平台登录账户和密码登录http://pay.weixin.qq.com// 平台设置的“API密钥”，为了安全，请设置为以数字和字母组成的32字符串。
    //添加商户密钥key字段
    [contentString appendFormat:@"key=%@", WechartSafeID];
